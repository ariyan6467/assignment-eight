...
const InstallApp = ({ app, installedApps, setInstalledApps }) => {
  ...
  const handleUninstall = (id) => {
    ...
    removeFromDb(id);
    const newArr = installedApps.filter((item) => item.id !== id);
  ...
  return (
    <div className="flex justify-between shadow-2xl items-center border-2 border-gray-300 rounded-md p-5 m-2">
      <div className="flex items-center gap-5">
        <img className="w-40 rounded-2xl" src={app.image} alt="" srcset="" />
      ...
      <div>
        <button onClick={() => handleUninstall(id)} className="btn btn-error">
          Uninstall
        </button>
